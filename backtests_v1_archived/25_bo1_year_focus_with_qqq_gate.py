...
QQQ_PATH = "data/qqq_dd52w.parquet"  # we’ll rebuild QQQ SMA200 inside this script from qqq_close
OUT_DIR = "data"
OUT_CSV = os.path.join(OUT_DIR, "bo1_year_focus_qqq_gate.csv")
...
def build_qqq_gate():
    ...
    q["qqq_gate_on"] = q["qqq_close"] > q["qqq_sma200"]
    return q[["date", "qqq_gate_on"]]

def build_events_for_ticker(g: pd.DataFrame, gate_map: dict) -> pd.DataFrame:
    ...
    for t in idx_breaks:
        # QQQ gate on breakout day
        dt = pd.Timestamp(g["date"].iloc[t]).normalize()
        if dt not in gate_map or (not gate_map[dt]):
            continue
...
def main():
    ...
    qqq_gate = build_qqq_gate()
    gate_map = {pd.Timestamp(d).normalize(): bool(v) for d, v in zip(qqq_gate["date"], qqq_gate["qqq_gate_on"])}
    ...
    for tkr, g in df.groupby("ticker", sort=False):
        ev = build_events_for_ticker(g, gate_map)
        if not ev.empty:
            ev["ticker"] = tkr
    ...
    if events.empty:
        raise RuntimeError("No BO-1 events produced with QQQ gate. (Gate may be too strict early in sample.)")
    ...
    print("BO-1 year focus WITH QQQ gate (net):")
