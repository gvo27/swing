...
def main():
    ...
    vals = pd.Series(list(qqq_risk_on.values()))
    print("QQQ risk_on fraction:", float(vals.mean()), "count:", int(vals.size), "false_count:", int((~vals).sum()))
    ...
    for di, d in enumerate(dates):
        ...
        # --- DEBUG: confirm TP gate behavior (temporary) ---
        if day.year in [2018, 2022] and day.month == 1 and day.day <= 10:
            print(f"{day.date()} | risk_on={risk_on}")
        ...
        for strat, picks in [("TP_v2", tp_pick), ("MR_1A", mr_pick), ("EB_1A", eb_pick)]:
            ...
            if (not risk_on) and len(tp_pick) > 0:
                print(day.date(), "RISK-OFF but TP picks exist:", len(tp_pick), "TP budget will be 0")
