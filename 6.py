...
def main():
        ...
        def enter_positions_mr(strat, picks, hold_days):
            ...
            for item, wi in zip(picks, w):
                ...
                if stress:
                    mr_entries_stress += 1
                    if mr_mode == "MR_P1":
                        mr_p1_entries_stress += 1
                    else:
                        mr_1a_entries_stress += 1
                else:
                    mr_entries_normal += 1

        enter_positions_tp("TP_v2", tp_pick, TP_HOLD)
        enter_positions_mr("MR_1A", mr_pick, mr_hold_today)   # MR budget bucket + hold for the chosen mode
    ...
    print("MR replacement sanity (entries by day-type):")
    print(f"  MR entries stress: {mr_entries_stress} | normal: {mr_entries_normal}")
    print(f"  On stress days -> MR_P1: {mr_p1_entries_stress} | MR_1A: {mr_1a_entries_stress}")
