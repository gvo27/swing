...
TP_DD_BUCKETS = [
    (-0.20, 0.00),  # dd < -0.20 => TP scale 0.00
    (-0.10, 0.50),  # -0.20 <= dd < -0.10 => TP scale 0.50
    ( 1.00, 1.00),  # dd >= -0.10 => TP scale 1.00  (1.00 here just means "anything above")
]
...
def main():
    ...
    for di, d in enumerate(dates):
        ...
        dd_today = qqq_dd_map.get(pd.Timestamp(d).normalize(), None)  # 'day' is your loop date variable
        tp_dd_scale = tp_scale_from_dd(dd_today)
