...
def load_env():
    ...
    required = {
        "date","ticker","open","close","high","low","atr14_pct","sma200","sma200_slope20",
        "ret1d","ret5d","ret3d","sma10","gap","day_ret"
    }
    ...
    return df, dates, date_to_i, frames_by_date, qqq_risk_on

# -----------------------------
# Paper trading helpers
# -----------------------------
PAPER_POS_PATH = os.path.join(OUT_DIR, "paper_positions.csv")
PAPER_TRADES_PATH = os.path.join(OUT_DIR, "paper_trades_next_open.csv")

def bday_add(d: pd.Timestamp, n: int) -> pd.Timestamp:
    # Approx trading days using business days. Good enough for paper planning.
    return (d + pd.tseries.offsets.BDay(n)).normalize()

def load_paper_positions() -> pd.DataFrame:
    if not os.path.exists(PAPER_POS_PATH):
        return pd.DataFrame(columns=["strategy","ticker","entry_date","exit_date","shares"])
    p = pd.read_csv(PAPER_POS_PATH)
    if p.empty:
        return pd.DataFrame(columns=["strategy","ticker","entry_date","exit_date","shares"])
...
def save_paper_positions(p: pd.DataFrame):
    p = p.copy()
    p.to_csv(PAPER_POS_PATH, index=False)

def run_paper_mode(account_equity_usd: float):
    _, dates, date_to_i, frames_by_date, qqq_risk_on = load_env()

    # Use the most recent date in the dataset as the signal date
    signal_day = pd.Timestamp(dates[-1]).normalize()
    today = frames_by_date.get(signal_day)
    if today is None or today.empty:
        raise RuntimeError("No data for last date in dataset.")

    risk_on = qqq_risk_on.get(signal_day.normalize(), True)

    # Approx next trading day for order placement
    next_open_day = bday_add(signal_day, 1)

    # Load existing paper positions (carry-forward)
    pos_df = load_paper_positions()

    # Drop positions that should already be exited as of signal_day (<= signal_day)
    if not pos_df.empty:
        pos_df = pos_df[pos_df["exit_date"] > signal_day].copy()

    # Current held tickers and strategy counts
    held_tickers = set(pos_df["ticker"].tolist()) if (not ALLOW_MULTIPLE_POS_PER_TICKER and not pos_df.empty) else set()
    active_tp = int((pos_df["strategy"] == "TP_v2").sum()) if not pos_df.empty else 0
    active_mr = int((pos_df["strategy"] == "MR_1A").sum()) if not pos_df.empty else 0
    active_eb = int((pos_df["strategy"] == "EB_1A").sum()) if not pos_df.empty else 0
    ...
    # Approx current invested value using today's close
    invested = 0.0
    if not pos_df.empty:
        for _, r in pos_df.iterrows():
            tkr = r["ticker"]
            if tkr in today.index:
                invested += float(r["shares"]) * float(today.loc[tkr, "close"])
    cash = max(0.0, float(account_equity_usd) - invested)

    # Build candidates using signal_day data only (no need for tomorrow open in dataset)
    tp_cands, mr_cands, eb_cands = [], [], []
    for tkr in today.index:
    ...
    # Budgets off account equity, then capped to available cash (paper realism)
    eq_now = float(account_equity_usd)
    budgets = {}
    ...
    def place_buys(strat, picks, hold_days):
        nonlocal cash, pos_df, orders
        if len(picks) == 0:
            return
        budget = float(budgets.get(strat, 0.0))
        if budget <= 0:
            return
        ...
        for (tkr, score, atr), wi in zip(picks, w):
            # We don't know next open; estimate using today's close
            px_ref = float(today.loc[tkr, "close"])
            fill_est = entry_fill(px_ref)
            dollars = budget * float(wi)

            # integer shares for live-like behavior
            shares = math.floor(dollars / fill_est)
            if shares <= 0:
                continue

            cost = shares * fill_est + COMMISSION_PER_TRADE_USD
            if cost > cash:
                continue

            # planned exit date (approx by business days from next open)
            exit_day = bday_add(next_open_day, hold_days)
            ...
            orders.append({
                ...
                "action": "BUY",
                "shares": int(shares),
                "est_entry_price": round(fill_est, 4),
                "notional_est": round(shares * fill_est, 2),
                "hold_days": int(hold_days),
                "planned_exit_date": exit_day.date(),
                "risk_on": bool(risk_on),
                "note": "MOO next session (price estimated from last close)",
            })
    ...
    place_buys("TP_v2", tp_pick, TP_HOLD)
    place_buys("MR_1A", mr_pick, MR_HOLD)
    place_buys("EB_1A", eb_pick, EB_HOLD)

    # Export orders + positions
    orders_df = pd.DataFrame(orders)
    orders_df.to_csv(PAPER_TRADES_PATH, index=False)
    save_paper_positions(pos_df)

    print("\n=== PAPER MODE (v1.1) ===")
    print(f"Signal date (latest in dataset): {signal_day.date()}")
    print(f"Next-open entry date (estimated): {next_open_day.date()}")
    print(f"QQQ risk_on: {risk_on}")
    print(f"Account equity: ${account_equity_usd:,.2f} | Invested est: ${invested:,.2f} | Cash est: ${cash:,.2f}")
    print(f"Orders exported: {len(orders_df)} -> {PAPER_TRADES_PATH}")
    print(f"Positions file: {PAPER_POS_PATH}")
...
def run_backtest_mode():
    os.makedirs(OUT_DIR, exist_ok=True)
    df, dates, date_to_i, frames_by_date, qqq_risk_on = load_env()
...
def main():
    ...
    parser.add_argument("--paper", action="store_true", help="Export next-open paper trades + paper positions")
    parser.add_argument("--equity", type=float, default=100000.0, help="Account equity in USD (paper mode sizing)")
    args = parser.parse_args()

    if args.paper:
        run_paper_mode(float(args.equity))
    else:
        run_backtest_mode()
