...
def mrp_signal(row) -> bool:
    if not np.isfinite(row.ret1d) or row.ret1d > MRP_RET1D_MAX:
        return False
    if not np.isfinite(row.ret5d) or row.ret5d > MRP_RET5D_MAX:
        return False
    if not np.isfinite(row.atr14_pct) or row.atr14_pct < MRP_ATR_MIN:
        return False
    return True
...
def build_market_stress_map(qqq_path: str, ret20d_thresh: float = -0.10) -> dict:
    q = pd.read_parquet(qqq_path).copy()
    ...
    q = q.sort_values("date").reset_index(drop=True)
    q["ret20d"] = q["qqq_close"].pct_change(20)
    q["market_stress"] = q["ret20d"] <= ret20d_thresh
    return {pd.Timestamp(d).normalize(): bool(v) for d, v in zip(q["date"], q["market_stress"].fillna(False))}
# -----------------------------
# Main
# -----------------------------
def main():
    ...
    stress_map = build_market_stress_map(QQQ_PATH, ret20d_thresh=-0.10)
    print("DEBUG stress_map type:", type(stress_map), "len:", len(stress_map))

    dates = pd.DatetimeIndex(sorted(df["date"].unique()))
    stress_days = sum(1 for d in dates if stress_map.get(pd.Timestamp(d).normalize(), False))
    print(f"Stress days: {stress_days} / {len(dates)} = {stress_days/len(dates):.2%}")
    ...
    for di, d in enumerate(dates):
        ...
        market_stress = stress_map.get(day.normalize(), False)
        if market_stress:
            mr_days_stress += 1
        else:
            mr_days_normal += 1
        ...
        for tkr in common:
            ...
            if market_stress:
                if mrp_signal(row):
                    # rank panic by depth (more negative ret5d first)
                    mr_cands.append((tkr, float(-row.ret5d), atr, "MRP"))
                    mr_cands_stress += 1
            else:
                if mr_signal(row, risk_on):
                    mr_cands.append((tkr, mr_rank(row), atr, "MR1A"))
                    mr_cands_normal += 1
            if eb_signal(row):
        ...
        def enter_positions(strat, picks, hold_days):
            nonlocal cash, positions, mr_entries, mr_entries_stress, mr_entries_normal, mr_entries_by_year
            if len(picks) == 0:
                return
            ...
            for p, wi in zip(picks, w):
                ...
                if strat == "MR_1A" and (mode in ("MRP","MR1A")):
                    if np.random.rand() < 0.0005:  # very rare sample
                        print("MR ENTRY", next_day.date(), tkr, "mode=", mode, "hold_days=", hold_days)
                ...
                if strat == "MR_1A":
                    ...
                    if mode == "MRP":
                        ...
                        mr_entries_normal += 1
                    y = int(pd.Timestamp(next_day).year)
                    mr_entries_by_year[y] = mr_entries_by_year.get(y, 0) + 1

        enter_positions("TP_v2", tp_pick, TP_HOLD)
        if market_stress:
            enter_positions("MR_1A", mr_pick, MRP_HOLD)
        else:
    ...
    print("\nMR entries:")
    print("  total:", mr_entries, "stress:", mr_entries_stress, "normal:", mr_entries_normal)
    print("  by year:", dict(sorted(mr_entries_by_year.items())))
