...
def main():
    ...
    regime_days = {"STRONG": 0, "NEUTRAL": 0, "RISK_OFF": 0}
    regime_open_sum = {"STRONG": 0, "NEUTRAL": 0, "RISK_OFF": 0}
    regime_invest_frac_sum = {"STRONG": 0.0, "NEUTRAL": 0.0, "RISK_OFF": 0.0}
    ...
    for i, day in enumerate(cal):
        ...
        if day in pending_entries:
            ...
            # Regime on ENTRY day (today)
            reg_entry = regime_map.get(day, "NEUTRAL")
            caps_entry = CAPS.get(reg_entry, CAPS["NEUTRAL"])
            exp_target = EXPOSURE.get(reg_entry, 0.60)

            target_invested = eq_now_for_sizing * exp_target
            current_invested = 0.0
            for p in positions:
                tkr = p["ticker"]
                if tkr in open_map and day in open_map[tkr].index:
                    opx = float(open_map[tkr].loc[day])
                    if np.isfinite(opx) and opx > 0:
                        current_invested += p["shares"] * opx

            remaining_budget = max(0.0, target_invested - current_invested)
            cap_total = min(MAX_TOTAL_POSITIONS, caps_entry["TP"] + caps_entry["MR"] + caps_entry["BO"])
            cap_total = max(1, cap_total)
            slots_remaining = max(1, cap_total - len(positions))

            target_alloc = remaining_budget / slots_remaining
        ...
        reg = regime_map.get(day, "NEUTRAL")
        if reg not in regime_days:
            reg = "NEUTRAL"
        ...
        for p in positions:
            ...
            if tkr in close_map and day in close_map[tkr].index:
                cpx = float(close_map[tkr].loc[day])
                if np.isfinite(cpx) and cpx > 0:
                    invested_close += p["shares"] * cpx

        invest_frac = invested_close / port_value if port_value > 0 else 0.0

        regime_days[reg] += 1
        regime_open_sum[reg] += len(positions)
        regime_invest_frac_sum[reg] += invest_frac
    ...
    for reg in ["STRONG", "NEUTRAL", "RISK_OFF"]:
        d = regime_days[reg]
        if d == 0:
            continue
        frac_days = d / total_days if total_days > 0 else 0.0
        avg_open = regime_open_sum[reg] / d
        avg_inv = regime_invest_frac_sum[reg] / d
        print(f"{reg}: days={d} ({frac_days:.1%}), avg_open_pos={avg_open:.2f}, avg_invested={avg_inv:.1%}")
