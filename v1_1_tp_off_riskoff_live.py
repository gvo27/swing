...
QQQ_PATH = "data/qqq_dd52w.parquet"  # must have date, qqq_close
...
def main():
    ...
    # Use the most recent date in your dataset as "today"
    today = pd.Timestamp(df["date"].max()).normalize()
    dates = sorted(df["date"].unique())
    di = dates.index(today)
    if di >= len(dates) - 1:
        raise RuntimeError("No 'tomorrow' available in dataset (need next trading day).")
    tomorrow = pd.Timestamp(dates[di + 1]).normalize()

    qqq_risk_on = build_qqq_regime_map(QQQ_PATH)
    risk_on = qqq_risk_on.get(today.normalize(), True)
    ...
    # Slice today's rows, only tickers that exist tomorrow too
    today_df = df[df["date"] == today].copy()
    tmr_df = df[df["date"] == tomorrow][["ticker"]].copy()
    ok = set(tmr_df["ticker"].astype(str).tolist())
    today_df = today_df[today_df["ticker"].astype(str).isin(ok)].copy()
    ...
    for (tkr, score, atr, close) in tp_pick:
        ...
        if shares <= 0:
            continue
        orders.append({
            "date_generated": str(today.date()),
            "intended_entry_date": str(tomorrow.date()),
            "ticker": tkr,
            "strategy": "TP_v2",
            "shares": shares,
            "hold_days": TP_HOLD,
    ...
    out_path = os.path.join(OUT_DIR, f"orders_{today.date()}.csv")
    orders_df.to_csv(out_path, index=False)

    print("LIVE v1.1 | TP_off_riskoff | 60/40 | 30 slots")
    print(f"Today: {today.date()}  Tomorrow: {tomorrow.date()}")
