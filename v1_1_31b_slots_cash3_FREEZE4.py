...
# -----------------------------
MAX_TOTAL_SLOTS = 30
CAP_W = {"TP_v2": 1.00, "MR_1A": 0.00, "EB_1A": 0.00}
SLOTS = {"TP_v2": 30, "MR_1A": 0, "EB_1A": 0}
...
def main():
    ...
    for di, d in enumerate(dates):
        ...
        for strat, picks in [("TP_v2", tp_pick), ("MR_1A", mr_pick), ("EB_1A", eb_pick)]:
            ...
            budgets[strat] = base
            if day.year == 2022 and day.month == 6 and day.day == 2 and strat in ("TP_v2","MR_1A"):
                print(f"[BUDGET] {day.date()} {strat} base={base:.2f} picks={len(picks)} per_pick={(base/len(picks)) if len(picks)>0 else 0:.2f}")
