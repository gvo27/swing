...
TP_DD_BUCKETS = [
    (-0.30, 0.25),  # dd < -30% => keep a small TP footprint
    (-0.20, 0.50),  # -30%..-20% => half size
    (-0.10, 0.80),  # -20%..-10% => slightly reduced
...
def main():
    ...
    for di, d in enumerate(dates):
        ...
        for strat, picks in [("TP_v2", tp_pick), ("MR_1A", mr_pick), ("EB_1A", eb_pick)]:
            ...
            base = eq_now * EFF_W[strat]

            # Keep your existing TP risk_off logic intact
            if strat == "TP_v2":
                if not risk_on:
                    # v1.1 behavior: no TP in risk-off
                    base = 0.0
                else:
                    # v2 addition: throttle TP size by QQQ drawdown scale during risk-on
                    base = base * float(tp_dd_scale)

            budgets[strat] = base
            if strat == "TP_v2" and day.year == 2022 and day.month in (6, 10) and day.day <= 5:
                print(f"[TP_BUDGET] {day.date()} risk_on={risk_on} tp_dd_scale={tp_dd_scale} base={base:.2f} picks={len(picks)}")
