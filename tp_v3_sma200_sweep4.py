...
# =========================
PARQUET_PATH = "data/sp100_daily_features.parquet2"
OUT_DIR = "research_outputs"
QQQ_PATH = "data/qqq_regime.parquet"
USE_QQQ_GATE_SWEEP = [False, True]   # compare off vs on
...
def tp_signal_mask(df: pd.DataFrame, min_above_sma200: float) -> pd.Series:
    ...
    # New v3 filter
    cond &= df["dist_above_sma200"].notna()
    cond &= df["dist_above_sma200"] >= min_above_sma200
...
def build_qqq_gate_map(path: str) -> dict:
    q = pd.read_parquet(path).copy()
    q["date"] = pd.to_datetime(q["date"]).dt.normalize()

    needed = ["date", "qqq_close", "qqq_sma200", "qqq_sma200_slope20"]
    missing = [c for c in needed if c not in q.columns]
    if missing:
        raise RuntimeError(f"qqq_regime missing columns: {missing}. Have: {q.columns.tolist()}")

    q["risk_on"] = (q["qqq_close"] > q["qqq_sma200"]) & (q["qqq_sma200_slope20"] > 0)
    return dict(zip(q["date"], q["risk_on"]))
...
def main():
    ...
    qqq_gate = build_qqq_gate_map(QQQ_PATH)
    ...
    ensure_columns(full, ["dist_above_sma200"])
    ...
    entry_dates = (pd.to_datetime(full["date"]).dt.normalize() + pd.tseries.offsets.BDay(1))
    risk_on_by_entry = entry_dates.map(qqq_gate).fillna(True)
    for use_gate in USE_QQQ_GATE_SWEEP:
        for x in MIN_ABOVE_SMA200_SWEEP:
            mask = tp_signal_mask(full, min_above_sma200=x)

            if use_gate:
                mask &= risk_on_by_entry
                risk_on_series = entry_dates.map(qqq_gate)
                mask &= risk_on_series.fillna(True)

            # ---- SANITY PRINT (ADD THIS BLOCK) ----
            da = full["dist_above_sma200"]
            print(
                f"\nmin_above_sma200={x:.2f} | "
                f"dist_above_sma200 stats: "
                f"min={da.min():.3f}, "
                f"p25={da.quantile(0.25):.3f}, "
                f"p50={da.median():.3f}, "
                f"p75={da.quantile(0.75):.3f}, "
                f"p95={da.quantile(0.95):.3f}"
            )
            print(f"  rows passing dist filter: {(da >= x).sum():,} / {len(da):,}")
            # ---- END SANITY PRINT ----
            ...
            if len(focus) > 0:
                ...
                print(f"\nTP_v3 | qqq_gate={use_gate} | min_above_sma200={x:.2f} | hold={HOLD_DAYS}D")
