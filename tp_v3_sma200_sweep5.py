...
DD_GATE_SWEEP = [None, -0.10, -0.15, -0.20, -0.25]  # None = no gate
...
def build_qqq_dd_map(path: str) -> dict:
    ...
    needed = ["date", "qqq_dd_from_52w_high"]
    missing = [c for c in needed if c not in q.columns]
    if missing:
        raise RuntimeError(f"qqq_dd52w missing columns: {missing}. Have: {q.columns.tolist()}")
...
def main():
    ...
    qqq_dd = build_qqq_dd_map(QQQ_DD_PATH)
    ...
    results = []
    year_tables = {}
    # Precompute entry dates + qqq dd series aligned to entry date
    entry_dates = (pd.to_datetime(full["date"]).dt.normalize() + pd.tseries.offsets.BDay(1))
    qqq_dd_entry = entry_dates.map(qqq_dd)  # may have NaNs near edges

    for dd_gate in DD_GATE_SWEEP:
        for x in MIN_ABOVE_SMA200_SWEEP:
            mask = tp_signal_mask(full, min_above_sma200=x)

            # Apply DD gate if specified
            if dd_gate is not None:
                mask &= (qqq_dd_entry >= dd_gate)

            # (your forward-return availability filters)
            ...
            if len(focus):
                print(f"\nTP_v3 | qqq_dd_gate={dd_gate} | min_above_sma200={x:.2f} | hold={HOLD_DAYS}D")
