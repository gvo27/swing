...
def main():
    ...
    for di, d in enumerate(dates):
        ...
        for strat, picks in [("TP_v2", tp_pick), ("MR_1A", mr_pick), ("EB_1A", eb_pick)]:
            ...
            if strat == "TP_v2":
                if not risk_on:
                    base *= float(TP_RISKOFF_SCALE)   # e.g. 0.25
                else:
                    base *= float(tp_dd_scale)

            budgets[strat] = base
            if strat == "TP_v2" and day.year == 2022 and day.month in (1, 6, 10):
                print(f"[TP_BUDGET] {day.date()} risk_on={risk_on} tp_dd_scale={tp_dd_scale:.2f} base={base:.2f} picks={len(picks)}")
